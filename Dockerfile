...
ARG COFFEESCRIPT_VERSION=2.2.3
ARG SASS_VERSION=3.5.6
ARG SASS_LISTEN_VERSION=4.0.0
...
RUN \
	apk add --no-cache \
		...
		ruby \
		ruby-dev
RUN gem install --no-ri --no-rdoc sass-listen -v "$SASS_LISTEN_VERSION" 
RUN gem install --no-ri --no-rdoc sass -v "$SASS_VERSION"
RUN npm install -g "coffeescript@$COFFEESCRIPT_VERSION"
